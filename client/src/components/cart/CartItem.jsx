        ...
        <div className="flex items-center justify-between mt-3">
          ...
          <div className="flex items-center border border-border">
            <button onClick={() => dispatch(updateQuantity({ key: item.key, quantity: item.quantity - 1 }))}
              ...
              className="w-8 h-8 flex items-center justify-center text-textSecondary hover:text-white disabled:opacity-30 transition-colors">−</button>
            <span className="w-8 text-center text-white text-sm">{item.quantity}</span>
            <button onClick={() => dispatch(updateQuantity({ key: item.key, quantity: item.quantity + 1 }))}
              className="w-8 h-8 flex items-center justify-center text-textSecondary hover:text-white transition-colors">+</button>
          </div>
          <div className="flex items-center gap-3">
            <span className="text-white text-sm font-semibold">{formatPrice(item.price * item.quantity)}</span>
            <button onClick={() => dispatch(removeFromCart(item.key))} className="text-textMuted hover:text-sale transition-colors" aria-label="Remove">
