...
const STATUS_COLORS = { pending: 'text-yellow-400 bg-yellow-400/10', confirmed: 'text-blue-400 bg-blue-400/10', shipped: 'text-purple-400 bg-purple-400/10', delivered: 'text-green-400 bg-green-400/10', cancelled: 'text-red-400 bg-red-400/10' };
...
export default function AdminOrders() {
  ...
  return (
    <div className="p-8">
      <h1 className="font-display text-4xl tracking-widest text-white mb-8">ORDERS</h1>

      {/* Filters */}
      <div className="flex flex-wrap gap-4 mb-6">
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search order number..." className="input-field max-w-xs" />
            ...
            <tbody>
              {orders.map(o => (
                <tr key={o._id} className="border-b border-border hover:bg-surfaceHover transition-colors">
                  ...
                  <td className="px-4 py-3"><span className={`text-xs font-semibold uppercase tracking-wider px-2 py-1 rounded ${STATUS_COLORS[o.status]}`}>{o.status}</span></td>
                  <td className="px-4 py-3"><span className={`text-xs ${o.paymentStatus === 'paid' ? 'text-success' : 'text-textMuted'}`}>{o.paymentStatus}</span></td>
                  <td className="px-4 py-3 text-textMuted text-xs">{new Date(o.createdAt).toLocaleDateString('en-IN')}</td>
                  <td className="px-4 py-3"><Link to={`/admin/orders/${o._id}`} className="text-xs text-textSecondary hover:text-white uppercase tracking-wider">View →</Link></td>
                </tr>
              ))}
              {!orders.length && <tr><td colSpan={8} className="px-6 py-12 text-center text-textMuted">No orders found</td></tr>}
            </tbody>
          </table>
        </div>
      )}
